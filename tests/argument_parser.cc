...
MODERNA_ADD_TEST(test_default_arg_is_flag) {
...
MODERNA_ADD_TEST(test_default_arg_required) {
...
MODERNA_ADD_TEST(test_multiple_arg_allowed) {
...
MODERNA_ADD_TEST(test_constraint_three_arg) {
...
MODERNA_ADD_TEST(test_required_imply_not_flag) {
  std::array argv = {"some_exec", "-v"};
  auto app = cli::app{app_id, argv.size(), argv.data()};
  app.add_argument("-v").required();
...
MODERNA_ADD_TEST(test_required_imply_single_arg) {
  std::array argv = {"some_exec", "-v", "1", "-v", "2"};
  auto app = cli::app{app_id, argv.size(), argv.data()};
  app.add_argument("-v").required();
  auto res = app.parse_args(false, false);
